...
默认的 `tok/s` 取自 transcript 里**最近已完成**的几轮生成（整轮结束才有 token 计数，所以是「上一轮」的速度）。要看**正在生成的这一轮此刻的瞬时速度**，启用本地测速代理：它在 Claude Code 和智谱之间透明转发请求，边转发边解析 SSE 流，把实时 output tok/s 写进状态文件供状态栏读取。

```bash
glm-status-line proxy install     # 装 launchd/macOS 或 systemd/Linux 服务，并改写 ANTHROPIC_BASE_URL 指向本地代理
```

装完**重启 Claude Code**，生成时状态栏的 `tok/s` 段就会实时跳动。卸载并还原：

```bash
glm-status-line proxy uninstall  # 停服务 + 还原 ANTHROPIC_BASE_URL
```

不想装服务也能前台跑（Ctrl+C 停止）：

```bash
glm-status-line proxy start [--port 7821] [--upstream https://open.bigmodel.cn/api/anthropic]
glm-status-line proxy status     # 看当前实时速率快照
```

**说明与安全**：

- 代理只监听 `127.0.0.1`，不对外；**不存储你的 token**（Claude Code 自己把 `Authorization` 头发给代理，代理原样转发给智谱）；只把 token 计数和速率写进本地状态文件，**不落盘任何内容**。
- 装了代理后 `ANTHROPIC_BASE_URL` 会变成 `http://127.0.0.1:7821`，本工具已做穿透：配额识别（`isGLM`）和配额接口地址仍按你原来的真实上游推导，**配额显示不受影响**。
- 代理没跑或没在生成时，`tok/s` 自动回退到 transcript 算法。
...
glm-status-line proxy install | uninstall        # 装服务 + 改写 base-url，详见上文「实时流式速度」
...
- **实时速度（tok/s）**：两种来源。① 默认走 transcript：取当前会话最近 2~3 轮已完成 assistant 消息，`output_tokens ÷ 该轮墙钟耗时` 求平均，10 秒缓存（整轮结束才有计数，故反映「上一轮」速度）。② 启用测速代理（`proxy install`）后，生成中优先读代理实时写入的瞬时速率——代理解析 SSE 流里 `message_delta` 的累计 `output_tokens`，按 ~2.5 秒滑窗算 tok/s。代理不可用时自动回退到 ①。
